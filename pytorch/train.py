...
# 2) результаты и целевые значения передаются в функцию потерь,
# чтобы вычислить значение потерь;
# 3) вычисляется градиент потерь по всем параметрам;
# 4) оптимизатор обновляет параметры согласно некоторому правилу.
# В PyTorch все это работает точно так же, за исключением двух небольших
# моментов:
# y по умолчанию оптимизаторы сохраняют градиенты параметров (param_
# grads) после каждой итерации обновления параметров. Чтобы очистить
# эти градиенты перед следующим обновлением параметра, нужно вызывать функцию self.optim.zero_grad;
# y как было показано ранее в простом примере с автоматическим дифференцированием, чтобы начать обратное распространение, нужно
# вызвать функцию loss.backward после вычисления значения потерь.
